...
def read_fasta_file(afile):
    ...
    # Trim sequences to remove Z and X at the end (if any)
    sequences = [(name, seq.rstrip('ZX')) for name, seq in sequences]

    # Remove sequences that have unorthodox amino acids
    sequences = [(name, seq) for name, seq in sequences if not set(seq) - amino_acids]
