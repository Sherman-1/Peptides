          ...
          line[13:16] == 'OT1'):
            ...
            if (math.fmod(line_counter,4) == 0 and line[17:20] != 'GLY' \
                ...
                cb_counter +=1
                 ...
                 and cb_counter >= num_cb):
                # Found non-gly residue with no CB atom
                # Change the name to GLY and retry
                return 1 
...
if __name__ == "__main__":
    ...
    files_paths = list(map(lambda x: Path(x), sys.argv[1:]))
    i = 0
    for file in tqdm(files_paths, leave = False, desc = "Computing descriptors"):
        ...
        try:
            ...
            if isinstance(result, int) and result == 1:
                ...
                s,Dt,Dr,vbar_prot,Rht,ffo_hyd_P,M,Ro,Rhr,int_vis,a_b_ratio,Ft,Rg,Dmax,tauC,asphr,int_vis,ks,AA,NA,GL,DT,useNumpy = result    

            df_dict['id'].append(basename)
            df_dict['s'].append(s)
            df_dict['Dt'].append(Dt)
            df_dict['Dr'].append(Dr)
            df_dict['vbar_prot'].append(vbar_prot)
            df_dict['Rht'].append(Rht)
            df_dict['ffo_hyd_P'].append(ffo_hyd_P)
            df_dict['M'].append(M)
            df_dict['Ro'].append(Ro)
            df_dict['Rhr'].append(Rhr)
            df_dict['int_vis'].append(int_vis)
            df_dict['a_b_ratio'].append(a_b_ratio)
            df_dict['Ft'].append(Ft)
            df_dict['Rg'].append(Rg)
            df_dict['Dmax'].append(Dmax)
            df_dict['tauC'].append(tauC)
            df_dict['asphr'].append(asphr)
            df_dict['ks'].append(ks)
            df_dict['AA'].append(AA)
            df_dict['NA'].append(NA)
            df_dict['GL'].append(GL)
            df_dict['DT'].append(DT)
    ...
    pl.DataFrame(df_dict).write_csv("bitopics_longs_hullrad.csv")
